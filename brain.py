...
class SQLiteMemory:
    """SQLite database interface for permanent SNN memory (Yaadasht)."""
    ...
    def _init_db(self):
        # Tables for Synapses, Neurons and Sequence Graph
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS neurons
                             (id INTEGER PRIMARY KEY, threshold REAL, gain REAL)''')
...
class SensoryEncoder:
    """Encoder jo strings ko spike sequences mein convert karta hai (Kaan)."""
...
class RealtimeBrain:
    ...
    def __init__(self, layer_sizes: list[int], density: float = 0.05, memory=None):
        ...
        # Load from SQLite if available (Structural Load)
        loaded = False
        if self.memory:
    ...
    def step(self, inputs: np.ndarray) -> np.ndarray:
        ...
        if not self.frozen:
            ...
            if self.learning_steps % 5 == 0: self._stdp_update_vectorized(spiking)
            self.eligibility *= 0.999
            self.depression = np.minimum(1.0, self.depression + 0.005)
            if self.learning_steps % 100 == 0: self._homeostasis_vectorized()
    ...
    def normalize_weights(self, target_avg: float = 0.8):
        avg = np.mean(self.w)
        if avg > 0: self.w = np.clip(self.w * (target_avg / avg), 0.01, 1.2)

    def save_memory(self):
        """Pure permanent storage using bulk inserts."""
        if not self.memory: return
        # Bulk Save Synapses
        syn_data = [(int(self.syn_pre[i]), int(self.syn_post[i]), float(self.w[i])) for i in range(len(self.w))]
        self.memory.cursor.executemany("INSERT OR REPLACE INTO synapses VALUES (?, ?, ?)", syn_data)

        # Bulk Save Neurons (Thr and Gain)
        neuron_data = [(i, float(self.thr[i]), float(self.gain[i])) for i in range(self.num_neurons)]
        self.memory.cursor.executemany("INSERT OR REPLACE INTO neurons VALUES (?, ?, ?)", neuron_data)

        self.memory.conn.commit()
        print(f"Memory Saved: {len(self.w)} synapses and {self.num_neurons} neurons stored in DB.")

    def load_memory(self):
        """Structural load: rebuilds the graph from DB records."""
        ...
        if len(rows) > 0:
            pre_ids, post_ids, weights = [], [], []
            for r in rows:
                pre_ids.append(r[0])
                post_ids.append(r[1])
                weights.append(r[2])
            self.syn_pre = np.array(pre_ids, dtype=int)
            self.syn_post = np.array(post_ids, dtype=int)
            ...
            # Load Neuron States
            self.memory.cursor.execute("SELECT id, threshold, gain FROM neurons")
            n_rows = self.memory.cursor.fetchall()
            for nid, thr, gain in n_rows:
                if nid < self.num_neurons:
                    self.thr[nid] = thr
                    self.gain[nid] = gain

            print(f"Memory Loaded: {num_syn}/{num_syn} saved weights recovered from DB.")
            return True
        return False
    ...
    def run_inference(self, ears, text, episodes=10):
        print(f"\n--- AI Brain Inference (Test Mode): '{text}' ---")
        self.frozen = True
        correct = 0
        ...
        for ep in range(episodes):
            ...
            for i, pattern in enumerate(sequence):
                ...
                if np.argmax(out) != i or np.sum(out) == 0: success = False
            ...
            for i, pattern in enumerate(sequence):
                ...
                if np.sum(out) > 0:
                    ...
                    if action == i:
                        ...
                        full_success = False
                        my_brain.v[my_brain.layer_indices[-1][action]] = my_brain.rest
                        my_brain.punish_neuron(my_brain.layer_indices[-1][action], amount=20.0)
                else:
                    full_success = False

                if full_success and i == 1:
                    success_seq = 1
                    if last_char and my_brain.memory:
                        ...
                        if pred: print(f"Recall: Maine ye sequence pehle dekha hai, {pred[0]} usually follows {last_char}.")
